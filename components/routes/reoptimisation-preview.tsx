...
export function ReoptimisationPreview({ route, candidate, onClose }: { route: ActiveRouteOperations; candidate: ReoptimisationCandidate; onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    try { await confirm({ routeId: route.route.id, candidateTaskId: candidate.id, expectedOrderedStopIds: preview.expectedOrderedStopIds as Id<"routeStops">[], expectedCurrentStopIndex: preview.expectedCurrentStopIndex }); onClose(); }
  ...
  return <Card><CardHeader><CardTitle>Review re-optimisation · {candidate.displayId}</CardTitle></CardHeader><CardContent className="space-y-4 text-sm">
    ...
    <div className="grid gap-4 md:grid-cols-2"><Order title="Current remaining order" ids={[route.stops.find((stop) => stop.isOperationalCurrent)?.taskId, ...preview.existingFutureOrder.map((id) => route.stops.find((stop) => stop.id === id)?.taskId)].filter((id): id is string => Boolean(id))} label={label} /><Order title="Proposed remaining order" ids={preview.completeProposedTaskOrder.slice(preview.expectedCurrentStopIndex)} label={label} moved={new Set(preview.movedTaskIds)} /></div>
