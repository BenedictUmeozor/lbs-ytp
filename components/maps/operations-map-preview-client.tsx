...
const TRUCK_ACTIVE_STATUSES = new Set(["on_route", "at_collection_point", "returning", "assigned"]);
...
export function OperationsMapPreviewClient({ depot, bins, reports, trucks, activeRoutePath }: OperationsMapData) {
  const center: [number, number] = depot ? [depot.latitude, depot.longitude] : FALLBACK_CENTER;
  ...
  return (
    ...
    >
      ...
      {activeRoutePath.length > 1 ? (
        <Polyline
          positions={activeRoutePath.map((point) => [point.latitude, point.longitude])}
      ...
      {depot ? (
        <CircleMarker
          ...
          radius={9}
          pathOptions={{ color: "#ffffff", weight: 2, fillColor: DEPOT_COLOR, fillOpacity: 1 }}
        />
      ) : null}
      ...
      {reports.map((report) => (
        <CircleMarker
          ...
          radius={6}
          pathOptions={{ color: "transparent", fillColor: REPORT_COLOR, fillOpacity: 0.9 }}
        />
      ))}
      ...
      {trucks.map((truck) => (
        <CircleMarker
          ...
          pathOptions={{
            color: "#ffffff",
            weight: 1.5,
            fillColor: TRUCK_ACTIVE_STATUSES.has(truck.status) ? TRUCK_ACTIVE_COLOR : TRUCK_IDLE_COLOR,
            fillOpacity: 0.95,
          }}
        />
      ))}
