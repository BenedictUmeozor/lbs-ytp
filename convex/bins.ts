...
function getLimit(limit: number | undefined, maximum: number) {
  ...
  if (!Number.isInteger(resolved) || resolved < 1 || resolved > maximum) throw new Error(`limit must be an integer between 1 and ${maximum}.`);
  return resolved;
}
...
export const listReadings = internalQuery({
  ...
  handler: async (ctx, args) => {
    const readings = await ctx.db.query("sensorReadings").withIndex("by_binId_and_recordedAt", (q) => q.eq("binId", args.binId)).order("desc").take(getLimit(args.limit, 200));
