...
import {
  buildReoptimisationStateSnapshot,
  calculateRemainingRouteMetrics,
  changedStopIds,
  isNearRemainingRoute,
  nextOperationalStopIndex,
  operationalCurrentStopIndex,
  proposedOrderedTaskIds,
  remainingRoutePointDistanceMeters,
  reoptimisationSnapshotsMatch,
  routeTask,
  splitRouteStops,
} from "./domain/route_reoptimisation";
import {
  canAssignRoute,
  canCancelRoute,
  canCompleteRoute,
  canConfirmReoptimisation,
  canEditProposedRoute,
  canStartRoute,
  isOpenRouteStatus,
  isTerminalRouteStopStatus,
  isUrgentReoptimisationCandidate,
} from "./domain/route_rules";
import {
  recalculateProposedRouteMetrics,
...
type RouteErrorCode =
  ...
  | "TASK_STATE_MISMATCH"
  | "ACTIVE_ROUTE_ALREADY_EXISTS"
  | "ROUTE_HAS_INCOMPLETE_STOPS"
  | "CANCELLATION_REASON_REQUIRED"
  | "REOPTIMISATION_UNAVAILABLE"
  | "REOPTIMISATION_CANDIDATE_INVALID"
  | "REOPTIMISATION_ROUTE_FULL"
  | "REOPTIMISATION_PREVIEW_STALE"
  | "REOPTIMISATION_CURRENT_STOP_UNAVAILABLE"
  | "ROUTE_RETURN_PENDING";
...
async function hasOpenRouteForTruck(
  ...
  const routes = await ctx.db
    ...
    .collect();
  return routes.some((route) => isOpenRouteStatus(route.status));
}
async function nextRouteDisplayId(ctx: MutationCtx) {
...
async function routeStops(ctx: QueryCtx | MutationCtx, routeId: Id<"routes">) {
  return ctx.db
    .query("routeStops")
...
async function routeTasks(
  ...
  if (tasks.some((task) => task === null))
    fail("TASK_UNAVAILABLE", "A route task is unavailable.");
  return tasks as Doc<"collectionTasks">[];
}
function cancellationReason(reason: string) {
  const value = reason.trim();
  if (value.length < 3 || value.length > 240)
    fail(
      "CANCELLATION_REASON_REQUIRED",
      "Enter a cancellation reason between 3 and 240 characters.",
    );
  return value;
}
  ...
  handler: async (ctx) => {
    ...
    return {
      settings: {
        ...
        effectiveMaximumStops: effectiveMaximumStops(
...
export const generateProposedRoute = mutation({
  args: { truckId: v.id("trucks"), taskIds: v.array(v.id("collectionTasks")) },
  handler: async (ctx, args) => {
    ...
    if (truck.status !== "available" || truck.assignedRouteId !== undefined)
    ...
    for (const task of tasks) {
      ...
      if (task.routeId !== undefined)
        fail(
          "TASK_ALREADY_ROUTED",
          "A selected task already belongs to a route.",
        );
      if (task.status !== "pending")
        fail("TASK_NOT_PENDING", "Selected tasks must still be pending.");
      if (!hasValidRouteCoordinates(task))
        fail(
          "INVALID_TASK_COORDINATES",
    ...
    const byId = new Map(selectedTasks.map((task) => [task._id, task]));
    const now = Date.now();
    for (const orderedTask of orderedTasks) {
      const task = byId.get(orderedTask.id);
      if (!task)
        fail("TASK_UNAVAILABLE", "A selected collection task is unavailable.");
    ...
    if (!route) throw new Error("Route creation failed.");
    ...
    return { routeId: route._id, displayId: route.displayId };
  },
});
...
export const getRouteDetail = query({
  ...
  handler: async (ctx, args) => {
    ...
    const orderedStops = await Promise.all(
      stops.map(async (stop, index) => {
        ...
        return {
          ...
          canMoveUp: canEditProposedRoute(route) && index > 0,
          canMoveDown: canEditProposedRoute(route) && index < stops.length - 1,
          canRemove: canEditProposedRoute(route) && stops.length > 1,
        };
      }),
    );
    const tasksTerminal = orderedStops.every((stop) =>
      ["collected", "unable_to_complete"].includes(stop.taskStatus),
    );
    const returnedToDepot =
      truck !== null &&
      truck.assignedRouteId === route._id &&
      truck.status === "returning" &&
      truck.latitude === route.depotLatitude &&
    ...
    const taskList = orderedStops.map((stop) => ({
      id: stop.taskId,
    ...
    return {
      route: {
        ...
        estimatedDistanceKm: route.estimatedDistanceKm,
        baseTravelMinutes:
          (route.estimatedDistanceKm / DEMO_AVERAGE_SPEED_KM_PER_HOUR) * 60,
        estimatedDurationMinutes: route.estimatedDurationMinutes,
        trafficPenaltyMinutes: route.trafficPenaltyMinutes,
        roadConditionPenaltyMinutes: route.roadConditionPenaltyMinutes,
        createdAt: route._creationTime,
      ...
      orderedStops,
      priorityComposition: priorityComposition(taskList),
      activityHistory,
      actions: {
        canEditStops: canEditProposedRoute(route),
        canAssign: canAssignRoute(route, stops.length),
        canStart: canStartRoute(route),
        canCancel: canCancelRoute(route),
        canComplete:
          canCompleteRoute(
            route,
...
export const moveProposedStop = mutation({
  args: {
    routeId: v.id("routes"),
    stopId: v.id("routeStops"),
  ...
  handler: async (ctx, args) => {
    ...
    if (!route) fail("ROUTE_UNAVAILABLE", "The route is unavailable.");
    if (!canEditProposedRoute(route))
      fail(
        "INVALID_ROUTE_TRANSITION",
        "Only proposed routes can be reordered.",
      );
    const stops = await routeStops(ctx, route._id);
    const index = stops.findIndex((stop) => stop._id === args.stopId);
    if (index < 0)
      fail("ROUTE_STOP_UNAVAILABLE", "The route stop is unavailable.");
    const swap = args.direction === "up" ? index - 1 : index + 1;
    if (swap < 0 || swap >= stops.length)
      fail(
        "INVALID_STOP_MOVE",
        "That stop cannot be moved further in this direction.",
      );
    [stops[index], stops[swap]] = [stops[swap], stops[index]];
    for (const [sequence, stop] of stops.entries())
      await ctx.db.patch(stop._id, { sequenceNumber: sequence + 1 });
    const update = await recalculateProposedRouteMetrics(ctx, route, stops);
    await insertActivityEvent(
      ctx,
      "route_order_changed",
      `Route ${route.displayId} stop order changed manually.`,
      "route",
      route._id,
      user._id,
    );
...
export const assignRoute = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!route) fail("ROUTE_UNAVAILABLE", "The route is unavailable.");
    const stops = await routeStops(ctx, route._id);
    if (!canAssignRoute(route, stops.length))
      fail(
        stops.length ? "INVALID_ROUTE_TRANSITION" : "ROUTE_HAS_NO_STOPS",
        stops.length
          ? "Only proposed routes can be assigned."
          : "A route needs at least one stop before assignment.",
      );
    const settings = await ctx.db
      .query("settings")
      .withIndex("by_key", (q) => q.eq("key", "global"))
    ...
    if (
      !settings ||
      stops.length > effectiveMaximumStops(settings.maximumRouteStops)
    )
      fail("TOO_MANY_TASKS", "This route exceeds the configured stop limit.");
    const truck = await ctx.db.get(route.truckId);
    if (!truck) fail("TRUCK_UNAVAILABLE", "The route truck is unavailable.");
    if (truck.status === "maintenance")
      fail("TRUCK_UNDER_MAINTENANCE", "Maintenance trucks cannot be assigned.");
    if (truck.status !== "available" || truck.assignedRouteId !== undefined)
      fail("TRUCK_STATE_MISMATCH", "The route truck is no longer available.");
    if (await hasOpenRouteForTruck(ctx, truck._id)) {
      ...
      if (
        open.some(
          (candidate) =>
            candidate._id !== route._id && isOpenRouteStatus(candidate.status),
        )
      )
        fail(
          "TRUCK_ALREADY_RESERVED",
          "This truck is reserved by another open route.",
        );
    }
    const tasks = await routeTasks(ctx, stops);
    for (const task of tasks)
      if (
        ...
        fail(
          "TASK_STATE_MISMATCH",
          "Every route task must still be scheduled only on this route.",
        );
    const now = Date.now();
    await ctx.db.patch(route._id, { status: "assigned" });
    ...
    for (const task of tasks) {
      ...
      await syncLinkedReportTaskStatus(ctx, task, "assigned", user._id, now);
      await insertActivityEvent(
        ctx,
        "task_status_changed",
        `Task ${task.displayId} assigned to ${truck.displayId}.`,
        "collection_task",
        task._id,
        user._id,
        "scheduled",
    ...
    await insertActivityEvent(
      ctx,
      "route_assigned",
      `Route ${route.displayId} assigned to ${truck.displayId}.`,
      "route",
      route._id,
      user._id,
      "proposed",
...
export const startRoute = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const route = await ctx.db.get(args.routeId);
    if (!route) fail("ROUTE_UNAVAILABLE", "The route is unavailable.");
    if (!canStartRoute(route))
      fail("INVALID_ROUTE_TRANSITION", "Only assigned routes can be started.");
    const active = await ctx.db
      .query("routes")
      .withIndex("by_status", (q) => q.eq("status", "active"))
      .first();
    if (active)
      fail("ACTIVE_ROUTE_ALREADY_EXISTS", "Another route is already active.");
    const [truck, stops] = await Promise.all([
      ctx.db.get(route.truckId),
      routeStops(ctx, route._id),
    ]);
    if (
      ...
      fail(
        "TRUCK_STATE_MISMATCH",
        "The route truck is not assigned to this route.",
      );
    if (!stops.length)
      fail(
        "ROUTE_HAS_NO_STOPS",
        "A route needs at least one stop before starting.",
      );
    const tasks = await routeTasks(ctx, stops);
    if (
      tasks.some(
      ...
      fail(
        "TASK_STATE_MISMATCH",
        "Every route task must be assigned to the route truck.",
      );
    const now = Date.now();
    const simulationVersion = getSimulationVersion(route) + 1;
    ...
    for (const [index, stop] of stops.entries())
      if (index === 0) await ctx.db.patch(stop._id, { status: "current" });
      else if (stop.status !== "pending")
        await ctx.db.patch(stop._id, { status: "pending" });
    for (const task of tasks) {
      await ctx.db.patch(task._id, {
        status: "en_route",
        statusUpdatedAt: now,
      });
      await syncLinkedReportTaskStatus(ctx, task, "en_route", user._id, now);
      await insertActivityEvent(
        ctx,
        "task_status_changed",
        `Task ${task.displayId} is en route.`,
        "collection_task",
        task._id,
        user._id,
        "assigned",
...
export const cancelProposedRoute = mutation({
  args: { routeId: v.id("routes"), reason: v.string() },
  handler: async (ctx, args) => {
    const { user } = await requireFleetManager(ctx);
    const reason = cancellationReason(args.reason);
    const route = await ctx.db.get(args.routeId);
    if (!route) fail("ROUTE_UNAVAILABLE", "The route is unavailable.");
    if (!canCancelRoute(route))
      fail(
        "INVALID_ROUTE_TRANSITION",
        "Only proposed routes can be cancelled.",
      );
    const stops = await routeStops(ctx, route._id);
    const tasks = await routeTasks(ctx, stops);
    if (
      tasks.some(
        (task) => task.status !== "scheduled" || task.routeId !== route._id,
      )
    )
      fail(
        "TASK_STATE_MISMATCH",
        "Only scheduled tasks on this proposal can be cancelled.",
      );
    const now = Date.now();
    for (const stop of stops) await ctx.db.delete(stop._id);
    for (const task of tasks)
    ...
    await insertActivityEvent(
      ctx,
      "route_cancelled",
      `Route ${route.displayId} cancelled: ${reason}`,
      "route",
      route._id,
      user._id,
      "proposed",
...
export const completeRoute = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (route.status !== "active")
      fail("INVALID_ROUTE_TRANSITION", "Only active routes can be completed.");
    const [truck, stops] = await Promise.all([
      ctx.db.get(route.truckId),
      routeStops(ctx, route._id),
    ]);
    if (
      ...
      fail("ROUTE_RETURN_PENDING", "Truck is still returning to the depot.");
    const tasks = await routeTasks(ctx, stops);
    if (
      !canCompleteRoute(
        route,
      ...
      fail(
        "ROUTE_HAS_INCOMPLETE_STOPS",
        "Complete or mark unable every route stop and task first.",
      );
    if (
      truck.latitude !== route.depotLatitude ||
      truck.longitude !== route.depotLongitude ||
    ...
    await insertActivityEvent(
      ctx,
      "route_completed",
      `Route ${route.displayId} completed.`,
      "route",
      route._id,
      user._id,
      "active",
...
function activeRouteMetrics(
  data: NonNullable<Awaited<ReturnType<typeof activeRouteData>>>,
) {
  ...
  return {
    ...
    metrics: calculateRemainingRouteMetrics(
      ...
      future,
      data.stops.length,
      data.stops.filter(({ stop }) => !isTerminalRouteStopStatus(stop.status))
        .length,
...
export const getActiveRouteOperations = query({
  args: {},
  handler: async (ctx) => {
    await requireFleetManager(ctx);
    ...
    const maximumStopCount =
      settings === null
        ? MAXIMUM_ROUTE_STOPS
        : effectiveMaximumStops(settings.maximumRouteStops);
        ...
        (
          await ctx.db
            ...
            .withIndex("by_status_and_priority", (q) =>
              q.eq("status", "pending").eq("priority", "critical"),
            )
            .collect()
        )
          ...
          .map(async (task) => {
            ...
            return {
              ...
              createdAt: task._creationTime,
              distanceMeters,
              isNearRoute: isNearRemainingRoute(distanceMeters),
              canReview: split.operationalCurrent !== null && capacityAvailable,
              unavailabilityReason: capacityAvailable
                ? undefined
                : "The active route is at its eight-stop limit.",
            };
          }),
      )
    ).sort(
      (a, b) =>
        Number(b.isNearRoute) - Number(a.isNearRoute) ||
    ...
    return {
      route: {
        id: data.route._id,
        displayId: data.route.displayId,
        status: data.route.status,
        depotLatitude: data.route.depotLatitude,
        depotLongitude: data.route.depotLongitude,
        estimatedDistanceKm: data.route.estimatedDistanceKm,
        estimatedDurationMinutes: data.route.estimatedDurationMinutes,
        trafficPenaltyMinutes: data.route.trafficPenaltyMinutes,
        roadConditionPenaltyMinutes: data.route.roadConditionPenaltyMinutes,
        startedAt: data.route.startedAt,
      },
      truck: {
        id: data.truck._id,
        displayId: data.truck.displayId,
        driverName: data.truck.driverName,
        status: data.truck.status,
        latitude: data.truck.latitude,
        longitude: data.truck.longitude,
        source: data.truck.source,
        locationLabel:
          data.truck.source === "simulated"
            ? "Simulated truck location"
            : "Static operational truck location",
      },
      stops: data.stops.map(({ stop, task }, index) => ({
        ...
        isRemaining:
          index >= currentIndex && !isTerminalRouteStopStatus(stop.status),
      })),
      progress: {
        totalStops: data.stops.length,
        completedStopCount: data.stops.filter(
          ({ stop }) => stop.status === "completed",
        ).length,
        unableStopCount: data.stops.filter(
          ({ stop }) => stop.status === "unable_to_complete",
        ).length,
        terminalStopCount: data.stops.filter(({ stop }) =>
          isTerminalRouteStopStatus(stop.status),
        ).length,
        remainingStopCount: data.stops.filter(
        ...
        nextStopId: nextIndex < 0 ? undefined : data.stops[nextIndex].stop._id,
        ...metrics,
      },
      simulation: {
        paused,
        version: getSimulationVersion(data.route),
        nextSimulationAt: data.route.nextSimulationAt,
        lastSimulationStepAt: data.route.lastSimulationStepAt,
        phase,
        targetLabel,
        currentTaskId: operationalStop?.task._id,
        canPause:
          !paused &&
          data.route.nextSimulationAt !== undefined &&
...
export const getReoptimisationPreview = query({
  args: { routeId: v.id("routes"), candidateTaskId: v.id("collectionTasks") },
  handler: async (ctx, args) => {
    await requireFleetManager(ctx);
    const data = await activeRouteData(ctx);
    const candidate = await ctx.db.get(args.candidateTaskId);
    if (data === null || data.route._id !== args.routeId)
      fail("REOPTIMISATION_UNAVAILABLE", "This route is not the active route.");
    if (candidate === null || !isUrgentReoptimisationCandidate(candidate))
      fail(
        "REOPTIMISATION_CANDIDATE_INVALID",
        "This critical task is no longer eligible for route review.",
      );
    const settings = await ctx.db
      .query("settings")
      .withIndex("by_key", (q) => q.eq("key", "global"))
    ...
    const maximumStopCount =
      settings === null
        ? MAXIMUM_ROUTE_STOPS
        : effectiveMaximumStops(settings.maximumRouteStops);
    if (data.stops.length >= maximumStopCount)
      fail(
        "REOPTIMISATION_ROUTE_FULL",
        "The active route is at its stop limit.",
      );
    const { split, metrics: currentRemainingMetrics } =
      activeRouteMetrics(data);
    if (split.operationalCurrent === null || currentRemainingMetrics === null)
      fail(
        "REOPTIMISATION_CURRENT_STOP_UNAVAILABLE",
        "The active route has no operational current stop.",
      );
    const proposedTaskIds = proposedOrderedTaskIds(data.stops, candidate);
    if (proposedTaskIds === null)
      fail(
        "REOPTIMISATION_CURRENT_STOP_UNAVAILABLE",
        "The active route has no operational current stop.",
      );
    ...
    const proposedTasks = proposedTaskIds
      .map((id) => taskById.get(id))
      .filter((task): task is Doc<"collectionTasks"> => task !== undefined);
    const proposedFuture = proposedTasks
      .slice(split.operationalCurrentIndex + 1)
      .filter((task) => task.status === "pending" || task._id === candidate._id)
      .map(routeTask);
    const proposedRemainingMetrics = calculateRemainingRouteMetrics(
      data.truck,
      split.operationalCurrent,
      proposedFuture,
      data.stops.length + 1,
    ...
    const distanceMeters = remainingRoutePointDistanceMeters(
      candidate,
      data.truck,
      split.operationalCurrent,
      data.stops,
    ...
    return {
      ...
      distanceMeters,
      isNearRoute: isNearRemainingRoute(distanceMeters),
      fixedTerminalStops: split.terminal.map(({ stop }) => stop._id),
      operationalCurrentStop: split.operationalCurrent.stop._id,
      existingFutureOrder: split.futurePending.map(({ stop }) => stop._id),
      proposedFutureOrder: proposedTaskIds.slice(
        split.operationalCurrentIndex + 1,
      ),
      completeProposedTaskOrder: proposedTaskIds,
      movedTaskIds: changedStopIds(currentTaskIds, proposedTaskIds),
      currentRemainingMetrics,
      proposedRemainingMetrics,
      currentFullRouteMetrics: {
        distanceKm: data.route.estimatedDistanceKm,
        durationMinutes: data.route.estimatedDurationMinutes,
      },
      proposedFullRouteMetrics: calculateRouteMetrics(
        {
          latitude: data.route.depotLatitude,
          longitude: data.route.depotLongitude,
        },
        proposedTasks.map(routeTask),
        data.route.trafficPenaltyMinutes,
        data.route.roadConditionPenaltyMinutes,
      ),
      explanation: `The operational current stop remains fixed. ${candidate.displayId} is added after it and only later pending stops are reordered.`,
      expectedOrderedStopIds: data.stops.map(({ stop }) => stop._id),
      expectedRouteState: buildReoptimisationStateSnapshot(data.stops),
      proposedTaskOrder: proposedTaskIds,
      expectedCurrentStopIndex: split.operationalCurrentIndex,
    };
  },
});

export const confirmReoptimisation = mutation({
  args: {
    routeId: v.id("routes"),
    candidateTaskId: v.id("collectionTasks"),
    expectedRouteState: v.array(reoptimisationStateEntryValidator),
    proposedTaskOrder: v.array(v.id("collectionTasks")),
    expectedCurrentStopIndex: v.number(),
    expectedCandidateLatitude: v.number(),
    expectedCandidateLongitude: v.number(),
  },
  handler: async (ctx, args) => {
    const { user } = await requireFleetManager(ctx);
    const data = await activeRouteData(ctx);
    ...
    if (
      data === null ||
      data.route._id !== args.routeId ||
      data.route.status !== "active" ||
      data.truck.status !== "on_route" ||
      data.truck.assignedRouteId !== data.route._id
    )
      fail(
        "REOPTIMISATION_UNAVAILABLE",
        "Route re-optimisation is unavailable.",
      );
    ...
    if (activeRoutes.length !== 1)
      fail(
        "REOPTIMISATION_UNAVAILABLE",
        "Route re-optimisation requires one active route.",
      );
    if (
      candidate === null ||
      !isUrgentReoptimisationCandidate(candidate) ||
      candidate.latitude !== args.expectedCandidateLatitude ||
      candidate.longitude !== args.expectedCandidateLongitude
    )
      fail(
        "REOPTIMISATION_CANDIDATE_INVALID",
        "This critical task is no longer eligible for route review.",
    ...
    const stale = () =>
      fail(
        "REOPTIMISATION_PREVIEW_STALE",
        "The route changed after this preview. Generate a new preview before confirming.",
      );
    ...
    const routeTasksConsistent = data.stops.every(
      ({ stop, task }) =>
    ...
    if (
      !routeTasksConsistent ||
      !reoptimisationSnapshotsMatch(currentSnapshot, args.expectedRouteState)
    )
      stale();
    const recalculatedProposal = proposedOrderedTaskIds(data.stops, candidate);
    if (currentIndex < 0 || recalculatedProposal === null)
      fail(
        "REOPTIMISATION_CURRENT_STOP_UNAVAILABLE",
        "The active route has no operational current stop.",
      );
    if (
      currentIndex !== args.expectedCurrentStopIndex ||
      recalculatedProposal.length !== args.proposedTaskOrder.length ||
      recalculatedProposal.some(
        (taskId, index) => taskId !== args.proposedTaskOrder[index],
      )
    )
      stale();
    const expectedTaskIds = new Set([
      ...data.stops.map(({ task }) => task._id),
      candidate._id,
    ]);
    const submittedTaskIds = new Set(args.proposedTaskOrder);
    if (
      submittedTaskIds.size !== args.proposedTaskOrder.length ||
      submittedTaskIds.size !== expectedTaskIds.size ||
      args.proposedTaskOrder.some((taskId) => !expectedTaskIds.has(taskId))
    )
      stale();
    ...
    await syncLinkedReportTaskStatus(
      ctx,
      candidate,
      "scheduled",
      user._id,
    ...
    await insertActivityEvent(
      ctx,
      "task_status_changed",
      `Task ${candidate.displayId} scheduled on ${data.route.displayId}.`,
      "collection_task",
      candidate._id,
      user._id,
      "pending",
    ...
    await ctx.db.patch(candidate._id, {
      assignedTruckId: data.truck._id,
      status: "assigned",
      statusUpdatedAt: now,
    });
    ...
    await insertActivityEvent(
      ctx,
      "task_status_changed",
      `Task ${candidate.displayId} assigned to ${data.truck.displayId}.`,
      "collection_task",
      candidate._id,
      user._id,
      "scheduled",
    ...
    await insertActivityEvent(
      ctx,
      "task_status_changed",
      `Task ${candidate.displayId} is en route.`,
      "collection_task",
      candidate._id,
      user._id,
      "assigned",
    ...
    const newStopId = await ctx.db.insert("routeStops", {
      routeId: data.route._id,
      taskId: candidate._id,
      sequenceNumber: data.stops.length + 1,
      status: "pending",
    });
    const stopIdByTaskId = new Map(
      data.stops.map(({ stop, task }) => [task._id, stop._id]),
    );
    stopIdByTaskId.set(candidate._id, newStopId);
    const orderedStopIds = args.proposedTaskOrder.map((taskId) =>
      stopIdByTaskId.get(taskId)!,
    );
    for (const [index, stopId] of orderedStopIds.entries())
      await ctx.db.patch(stopId, { sequenceNumber: index + 1 });
    const orderedTasks = args.proposedTaskOrder.map((taskId) =>
      taskById.get(taskId)!,
    );
    const fullMetrics = calculateRouteMetrics(
      {
        latitude: data.route.depotLatitude,
        longitude: data.route.depotLongitude,
      },
      orderedTasks.map(routeTask),
      data.route.trafficPenaltyMinutes,
      data.route.roadConditionPenaltyMinutes,
    );
    await ctx.db.patch(data.route._id, {
      orderedStopIds,
      currentStopIndex: orderedStopIds.indexOf(
        data.stops[currentIndex].stop._id,
      ),
      estimatedDistanceKm: fullMetrics.totalDistanceKm,
      estimatedDurationMinutes: fullMetrics.estimatedDurationMinutes,
    });
    const notifications = await ctx.db
      ...
      .withIndex("by_relatedEntityType_and_relatedEntityId", (q) =>
        q
          ...
          .eq("relatedEntityId", candidate._id),
      )
      .collect();
    for (const notification of notifications)
      if (
        notification.type === "route_reoptimisation_suggested" &&
        notification.readAt === undefined
      )
        await ctx.db.patch(notification._id, { readAt: now });
    const near = isNearRemainingRoute(
      remainingRoutePointDistanceMeters(
        candidate,
        data.truck,
        split.operationalCurrent!,
    ...
    await insertActivityEvent(
      ctx,
      "route_task_linked",
      `Task ${candidate.displayId} linked to ${data.route.displayId}.`,
      "collection_task",
      candidate._id,
      user._id,
    );
    await insertActivityEvent(
      ctx,
      "route_reoptimised",
      `Manager confirmed ${candidate.displayId} ${near ? "near the remaining route" : "outside the 1 km route-review radius"} for ${data.route.displayId}.`,
      "route",
      data.route._id,
      user._id,
    );
