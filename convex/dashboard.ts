...
async function getActiveRouteRecord(ctx: QueryCtx): Promise<ActiveRouteRecord | null> {
...
const overviewMapValidator = v.object({
  ...
  activeRoutePath: v.array(v.object({ latitude: v.number(), longitude: v.number() })),
});

async function buildOverviewMap(ctx: QueryCtx, activeRouteRecord: ActiveRouteRecord | null) {
  const [settings, bins, reports, trucks] = await Promise.all([
    ctx.db.query("settings").withIndex("by_key", (q) => q.eq("key", "global")).unique(),
    ctx.db.query("bins").collect(),
    ctx.db.query("citizenReports").collect(),
    ctx.db.query("trucks").collect(),
  ...
  const depot = settings === null ? null : {
  ...
  const activeRoutePath = depot === null || activeRouteRecord === null
    ? []
    : [
        { latitude: depot.latitude, longitude: depot.longitude },
        ...activeRouteRecord.stops.map(({ task }) => ({ latitude: task.latitude, longitude: task.longitude })),
      ];
...
const criticalAlertSeverityValidator = v.union(v.literal("critical"), v.literal("warning"));
...
async function buildCriticalAlerts(ctx: QueryCtx) {
  ...
  const relevant = unread.filter(
    (notification): notification is typeof notification & { severity: "critical" | "warning" } =>
      notification.severity === "critical" || notification.severity === "warning",
  );
  relevant.sort((left, right) => severityRank[left.severity] - severityRank[right.severity] || right._creationTime - left._creationTime);
...
async function buildCollectionProgress(ctx: QueryCtx) {
  ...
  const pendingCount = nonCancelled.filter((task) => task.status === "pending").length;
  const activeCount = nonCancelled.filter((task) => task.status === "scheduled" || task.status === "assigned" || task.status === "en_route").length;
  const collectedCount = nonCancelled.filter((task) => task.status === "collected").length;
  const unableToCompleteCount = nonCancelled.filter((task) => task.status === "unable_to_complete").length;
  const totalCount = nonCancelled.length;
  const completionPercentage = totalCount === 0 ? 0 : Math.round((collectedCount / totalCount) * 100);

  return { totalCount, pendingCount, activeCount, collectedCount, unableToCompleteCount, completionPercentage };
}
...
async function buildActiveRouteSummary(ctx: QueryCtx, activeRouteRecord: ActiveRouteRecord | null) {
  if (activeRouteRecord === null) return null;
  const { route, stops } = activeRouteRecord;

  const truck = await ctx.db.get(route.truckId);
  if (truck === null) throw new Error(`Route ${route._id} references a missing truck.`);
...
const maintenanceRiskOrder: Record<MaintenanceRisk, number> = { high: 0, medium: 1, normal: 2 };

async function buildVehicleHealth(ctx: QueryCtx) {
  const trucks = await ctx.db.query("trucks").collect();
  const sorted = [...trucks].sort((left, right) => maintenanceRiskOrder[left.maintenanceRisk] - maintenanceRiskOrder[right.maintenanceRisk]);
...
export const getOverviewData = query({
  ...
  handler: async (ctx) => {
    ...
    const activeRouteRecord = await getActiveRouteRecord(ctx);

    const [summary, map, criticalAlerts, recentActivity, collectionProgress, activeRoute, vehicleHealth] = await Promise.all([
      computeOverviewSummary(ctx),
      buildOverviewMap(ctx, activeRouteRecord),
    ...
    return { summary, map, criticalAlerts, recentActivity, collectionProgress, activeRoute, vehicleHealth };
  },
});
