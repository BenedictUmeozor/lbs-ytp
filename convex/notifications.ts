...
function getLimit(limit: number | undefined) {
  ...
  if (!Number.isInteger(resolved) || resolved < 1 || resolved > 100) throw new Error("limit must be an integer between 1 and 100.");
  return resolved;
}

export const list = internalQuery({
  args: { limit: v.optional(v.number()) },
  handler: (ctx, args) => ctx.db.query("notifications").order("desc").take(getLimit(args.limit)),
});
...
export const markRead = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (notification.readAt !== undefined) return { changed: false, readAt: notification.readAt };
