...
function validateSettings(settings: SettingsValues) {
  ...
  if (!(
    0 <= emptyConfirmationThreshold &&
    emptyConfirmationThreshold < approachingFullThreshold &&
    approachingFullThreshold < collectionRequiredThreshold &&
    collectionRequiredThreshold < criticalThreshold &&
    criticalThreshold <= 100
  )) {
    throw new Error(
      "Thresholds must satisfy 0 <= empty < approaching < collection required < critical <= 100.",
    );
  }

  requireIntegerInRange(
    "deviceOfflineTimeoutMinutes",
    settings.deviceOfflineTimeoutMinutes,
    1,
    60,
  );
  requireIntegerInRange(
    "duplicateDistanceThresholdMeters",
    settings.duplicateDistanceThresholdMeters,
    1,
    5000,
  );
  requireIntegerInRange("maximumRouteStops", settings.maximumRouteStops, 1, 8);
  if (settings.depotLatitude < -90 || settings.depotLatitude > 90)
    throw new Error("depotLatitude must be between -90 and 90.");
  if (settings.depotLongitude < -180 || settings.depotLongitude > 180)
    throw new Error("depotLongitude must be between -180 and 180.");
  requireIntegerInRange(
    "simulationStepIntervalSeconds",
    settings.simulationStepIntervalSeconds,
    1,
    60,
  );
  requireIntegerInRange(
    "hardwareDemoIntervalSeconds",
    settings.hardwareDemoIntervalSeconds,
    1,
    300,
  );
  requireIntegerInRange(
...
export const update = internalMutation({
  ...
  handler: async (ctx, args) => {
    const settings = await ctx.db
      ...
      .unique();
    if (settings === null)
      throw new Error("Global settings record does not exist.");

    const suppliedEntries = (
      Object.entries(args) as [keyof SettingsValues, number | undefined][]
    ).filter(
      (entry): entry is [keyof SettingsValues, number] =>
        entry[1] !== undefined,
    );
    ...
    const merged: SettingsValues = {
      approachingFullThreshold:
        args.approachingFullThreshold ?? settings.approachingFullThreshold,
      collectionRequiredThreshold:
        args.collectionRequiredThreshold ??
        settings.collectionRequiredThreshold,
      criticalThreshold: args.criticalThreshold ?? settings.criticalThreshold,
      emptyConfirmationThreshold:
        args.emptyConfirmationThreshold ?? settings.emptyConfirmationThreshold,
      deviceOfflineTimeoutMinutes:
        args.deviceOfflineTimeoutMinutes ??
        settings.deviceOfflineTimeoutMinutes,
      duplicateDistanceThresholdMeters:
        args.duplicateDistanceThresholdMeters ??
        settings.duplicateDistanceThresholdMeters,
      ...
      simulationStepIntervalSeconds:
        args.simulationStepIntervalSeconds ??
        settings.simulationStepIntervalSeconds,
      hardwareDemoIntervalSeconds:
        args.hardwareDemoIntervalSeconds ??
        settings.hardwareDemoIntervalSeconds,
      trafficPenaltyMinutes:
        args.trafficPenaltyMinutes ?? settings.trafficPenaltyMinutes,
      roadConditionPenaltyMinutes:
        args.roadConditionPenaltyMinutes ??
        settings.roadConditionPenaltyMinutes,
    };
    validateSettings(merged);
    const changedFields = suppliedEntries
      .filter(([name, value]) => settings[name] !== value)
      .map(([name]) => name);
    if (changedFields.length === 0)
      return { changed: false, settingsId: settings._id, changedFields };

    await ctx.db.patch(
      settings._id,
      Object.fromEntries(changedFields.map((name) => [name, merged[name]])),
    );
    await insertActivityEvent(
      ctx,
      "settings_updated",
      `Settings updated: ${changedFields.join(", ")}.`,
      "settings",
      settings._id,
    );
